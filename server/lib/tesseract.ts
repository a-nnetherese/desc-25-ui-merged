...
function parseGroceryItems(text: string): GroceryItem[] {
  ...
  for (const line of lines) {
    ...
    // Extract quantity with unit (handles decimals, fractions, and mixed numbers)
    // Matches: "2 lbs", "1.5 cups", "1/2 tsp", "1 1/2 cups"
    const quantityUnitMatch = line.match(/^((\d+\s+)?(\d+)\/(\d+)|\d+(?:\.\d+)?)\s*(lbs?|oz|cups?|tbsp|tsp|g|kg|ml|l|gallons?|quarts?|pints?|items?)?/i);
    let quantity = "1 item";
    let itemName = line;

    if (quantityUnitMatch) {
      const fullQuantity = quantityUnitMatch[1]; // Could be "1.5", "1/2", or "1 1/2"
      const unit = quantityUnitMatch[5] || "item";
      quantity = `${fullQuantity} ${unit}`;
      // Remove the quantity and unit from item name
      itemName = line.substring(quantityUnitMatch[0].length).trim();
    } else {
      // Fallback: try to find simple quantity without unit
    ...
    // Clean item name - remove prices and special characters but keep letters and spaces
    itemName = itemName
